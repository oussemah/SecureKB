...
void test_vdd(void)
{
  ...
  /* Display test name*/
  LCD_GLASS_DisplayString((uint8_t*)" VDD ");
  DELAY;
  ...
  if ((vdd_test>VCC_MAX) || (vdd_test<VCC_MIN))
  {
    ...
    while(1)
    {
      ...
      LCD_GLASS_ScrollSentence((uint8_t*)"VDD ERROR ",1,SCROLL_SPEED); 
      DELAY;
  ...
  LCD_GLASS_DisplayString((uint8_t*)"VDD OK");
  DELAY ;
}
...
void test_icc_Run(void)
{
  ...
  /* Display test name*/
  LCD_GLASS_DisplayString((uint8_t*)"RUN   ");
  DELAY;
  ...
  if ((icc_test>ICC_RUN_MAX) || (icc_test<ICC_RUN_MIN))
  {
    ...
    while (1)
    {
      ...
      LCD_GLASS_ScrollSentence((uint8_t*)"RUN ERROR ",1,SCROLL_SPEED); 
      DELAY;
  ...
  LCD_GLASS_DisplayString((uint8_t*)"RUN OK");
  DELAY;
}
...
void test_Bias(void)
{
  float Current = 0;
  /* Display test name*/
  LCD_GLASS_DisplayString((uint8_t*)"BIAS   ");
  DELAY;
  ...
  if ((Bias_Current > ICC_BIAS_MAX) || (Bias_Current == 0 ))
  {
    ...
    while (1)
    {
      ...
      LCD_GLASS_ScrollSentence((uint8_t*)"BIAS ERROR ",1,SCROLL_SPEED);
      DELAY;
      display_MuAmp((uint32_t)Current);
      DELAY;
      DELAY;
  ...
  LCD_GLASS_DisplayString((uint8_t*)"BIASOK");
  DELAY;
}
...
void test_icc_STOP(void)
{
  ...
  /* Test if value is correct */
    if ((icc_test>ICC_STOP_MAX) || (icc_test<ICC_STOP_MIN))
    {
      ...
      while (1)
      {
        ...
        LCD_GLASS_ScrollSentence((uint8_t*)"ICC STOP ERROR ",1,SCROLL_SPEED);
        DELAY;
  ...
  LCD_GLASS_DisplayString((uint8_t*)"STOPOK");
  DELAY;
}
...
{
  /* Display test name*/
  LCD_GLASS_DisplayString((uint8_t*)"STBY  ");
  DELAY;
...
void auto_test(void)
{
  ...
  /* To display version */
  LCD_GLASS_DisplayString((uint8_t*)" TEST ");
  DELAY;
  STR_VERSION;
  ...
  while (1)
  {
    LCD_GLASS_ScrollSentence((uint8_t*)"TEST OK ",1,SCROLL_SPEED);
    KeyPressed = FALSE;
  }
}
 ...
 /* Test if value is correct */
    if ((Current_STBY > ICC_STBY_MAX) || (Current_STBY < ICC_STBY_MIN))
    {
      ...
      while (1)
      {
        ...
        LCD_GLASS_ScrollSentence((uint8_t*)"ICC STBY ERROR ",1,SCROLL_SPEED); 
        DELAY;
  ...
  LCD_GLASS_DisplayString((uint8_t*)"STBYOK");
  DELAY;    
...
void Bias_measurement(void)
{
  ...
  LCD_GLASS_ScrollSentence((uint8_t*)"      ** BIAS CURRENT ** JP1 OFF **",1,SCROLL_SPEED);	
